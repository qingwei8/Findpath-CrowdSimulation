...
void displayCrowdDistribution(hiveCrowdRendering::CVisualizationByOpenCV* vDisplay, hiveCrowdSimulation::CCrowdSimulation* vCrowdSimulation)
{
	...
	std::vector<hiveCrowdSimulation::CDerivedAgent*> TotalAgentSet;
	vCrowdSimulation->dumpTotalAgent(TotalAgentSet);

	glm::vec3 AgentColor = glm::vec3(0, 255, 0);
	for (unsigned int i=0; i<TotalAgentSet.size(); ++i)
	{
		hiveCrowdSimulation::CDerivedAgent* pAgent = TotalAgentSet[i];
		if (pAgent->isAware())
		{
			vDisplay->drawPoint(pAgent->getPosition(), 4, AgentColor, true);
		}
		else
		{
			vDisplay->drawPoint(pAgent->getPosition(), 4, AgentColor, false);
		}
	}
}
